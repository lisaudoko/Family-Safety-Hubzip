...
- [x] Rule evaluation engine — _completed 2026-08-08: `GET /api/family/children/:childId/effective-policy` (`services/effectivePolicy.ts`) resolves family → child → (optional, via `?deviceId=`) device precedence for nullable fields, and OR-combines boolean flags across whichever layers have a row (see `docs/POLICY_ENGINE.md` for why booleans use OR instead of most-specific-wins). This is resolution only — nothing on-device reads or applies the resolved answer. Mobile: a "What Currently Applies" summary card on `app/child/[id].tsx`, explicitly labeled "not yet enforced on this device."_
- [x] Policy conflict resolution — _same 2026-08-08 change; "conflict resolution" in practice meant precedence ordering, which the engine above implements._

### Device Enforcement (Not Started — requires the native phase below)

_Product-owner decision 2026-08-08: build all the non-native groundwork first (effective-policy engine above, family-wide policy mobile UI, clearer online/last-active device status), and treat everything below as a deferred, explicitly separate phase gated on pursuing Apple's Family Controls entitlement. This app is pure Expo-managed today (no `eas.json`, no dev-client plugin, no native modules) — that has to change before any of the below is possible on iOS._

- [ ] **Native phase prerequisite**: request Apple's `com.apple.developer.family-controls` entitlement (weeks-long approval, not guaranteed) and migrate off Expo Go to a custom dev client / EAS builds.
- [ ] Cross-platform enforcement agent (iOS & Android)
- [ ] App install blocking — iOS: via Screen Time API (`ManagedSettings`) once the entitlement above is granted
- [ ] App usage schedules & time limits — _definition/storage exists (`device_app_rules`, see Policy Definition above), and the resolved answer now exists too (`getEffectivePolicy`); actual on-device enforcement still requires the native phase, no agent exists_
- [ ] Device lock schedules (bedtime/school hours) — same: definition + resolution exist, enforcement doesn't
- [ ] Real per-app usage ("most time spent on apps") — _confirmed 2026-08-08: not real today. `screen_time` device events only measure Digital Village's own foreground time (`lib/deviceSync.ts`'s own comment says so); the `"activity"` event type has zero producers anywhere in the mobile app. Product-owner decision: eventual implementation should use Apple's `DeviceActivityReport` extension (on-device rendering only, per Apple's privacy model — not freely exportable to our backend), part of the native phase._
- [ ] Website & content filtering
- [ ] Remote device controls
- [ ] Parent alerts & activity monitoring
- [ ] Tamper detection & prevention
- [ ] Location tracking — _explicitly deferred 2026-08-08 (product-owner decision): architecturally separate from Screen Time work (CoreLocation background permission, its own heavy App Store privacy review); not scoped further yet._
...
- ~~Per-child devices/monitoring on Family tab~~ — implemented 2026-07-10: each child's detail screen (`app/child/[id].tsx`) now has its own "Default Restrictions" (bound to the `child_policies` endpoints) and "Devices" section (rename in place, expand to per-device restrictions), closing the mobile-UI gap flagged in the prior Family & Child Policies changelog entry. Removed the device-picker/restrictions UI from `app/(tabs)/profile.tsx`.
- ~~Family-wide policy mobile UI~~ — implemented 2026-08-08: new `app/family-policy.tsx` ("Family Defaults", entry point from the parent Family tab), backed by new `hooks/useFamilyPolicy.ts` and `apiGetFamilyPolicy`/`apiUpdateFamilyPolicy` in `lib/apiClient.ts` (the mobile API client wrapper didn't exist before this either, even though the backend route did). Closes the gap flagged in the prior entry.
